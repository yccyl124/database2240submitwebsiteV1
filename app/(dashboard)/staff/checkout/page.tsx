...
export default function CheckoutPOS() {
  ...
  // --- REFINED PROCESS SALE (WITH GUARANTEED RESET) ---
  const processSale = async (methodId: string) => {
    ...
    try {
      // 1. Create Transaction Master Record
      ...
      if (txnError) throw new Error(`TXN failed: ${txnError.message}`);

      // 2. Process each item (Stock Deduction)
      for (const item of cart) {
        ...
        // b. Update Global Batch Table
        await supabase.from('batches')
          .update({ remainingqty: item.available - item.qty })
          .eq('batchid', item.batchid);

        // c. Update Local Inventory Table (Checks if record exists first to prevent crash)
        const { data: inv } = await supabase.from('inventory')
          .select('quantity')
          .eq('batchid', item.batchid)
          .eq('locationid', parseInt(selectedLocationId.toString()))
          .maybeSingle(); // maybeSingle doesn't throw error if missing
      ...
      // --- 4. THE RESET (This is now outside the catch logic to ensure it ALWAYS happens) ---
      setCart([]);
      setCustomer(null);
      setCustomerPhone('');
      setAppliedPromo(null);
